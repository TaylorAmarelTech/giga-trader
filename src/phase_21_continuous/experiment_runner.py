            ...
            if skip_augmentation:
                ...
                try:
                    df_train_augmented, metadata = integrate_anti_overfit(
                        ...
                        use_expanded_macro=getattr(config.anti_overfit, 'use_expanded_macro', True),
                        use_vvix=getattr(config.anti_overfit, 'use_vvix', True),
                        use_sector_rotation=getattr(config.anti_overfit, 'use_sector_rotation', True),
                        use_fx_carry=getattr(config.anti_overfit, 'use_fx_carry', True),
                        use_money_market=getattr(config.anti_overfit, 'use_money_market', True),
                        use_financial_stress=getattr(config.anti_overfit, 'use_financial_stress', True),
                        use_global_equity=getattr(config.anti_overfit, 'use_global_equity', True),
                        use_retail_sentiment=getattr(config.anti_overfit, 'use_retail_sentiment', True),
                        use_cboe_pcr=getattr(config.anti_overfit, 'use_cboe_pcr', True),
                        use_stocktwits=getattr(config.anti_overfit, 'use_stocktwits', True),
                        use_alpaca_news=getattr(config.anti_overfit, 'use_alpaca_news', True),
                        use_gnews_headlines=getattr(config.anti_overfit, 'use_gnews_headlines', True),
                        use_finbert_nlp=getattr(config.anti_overfit, 'use_finbert_nlp', False),
                        use_wsb_sentiment=getattr(config.anti_overfit, 'use_wsb_sentiment', False),

...
def api_performance():
    """Get paper trading performance (real-world model accuracy)."""
    ...
    if perf_file.exists():
        try:
            ...
            return jsonify({"error": str(e), "summary": {"total_trades": 0}})
    return jsonify({"summary": {"total_trades": 0}, "predictions": []})
    ...
    <script>
        ...
        async function updateExperiment() {
            try {
                ...
                const expectedTypical = data.step_expected_typical || 300;
                const expectedMax = data.step_expected_max || 1800;
                ...
                if (data.is_running) {
                    if (data.stuck_critical) {
                        ...
                        activityEl.querySelector('.activity-text').textContent = `May be stuck (${formatDuration(secsSinceActivity)} since last update)`;
                    } else if (data.stuck_warning) {
                        activityEl.className = 'activity-indicator warning';
                        activityEl.querySelector('.activity-text').textContent = `Long step (${formatDuration(secsSinceActivity)} — typically up to ${formatDuration(expectedMax)})`;
                    } else if (secsSinceActivity > 120) {
                        // More than 2 min but within expected range — show working indicator
                        activityEl.className = 'activity-indicator active';
                        activityEl.querySelector('.activity-text').textContent = `Working (${formatDuration(secsSinceActivity)} — this step can take up to ${formatDuration(expectedMax)})`;
                    } else {
                        activityEl.className = 'activity-indicator active';
